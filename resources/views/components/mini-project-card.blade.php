    ...
    <div class="flex flex-col gap-4">
        <div class="flex flex-wrap gap-2">
            @foreach ($project['tech_stack'] as $tech)
                ...
                <span class="px-3 py-1 text-xs bg-{{ $color }}/10 text-{{ $color }} rounded-full ">
                    {{ $tech }}
                </span>
            @endforeach
        </div>
        <h4 class="font-display text-xl font-semibold !leading-[1.25] max-sm:text-lg text-text">{{ $project['title'] }}
        </h4>
        <p class="text-sm text-grayBright"> {{ $project['desc'] }} </p>
        <span class="w-fit text-sm text-teal flex items-center gap-2">Read
            more
            <i
